...
implements IBlockStatePaletteResizer {
    ...
    private void setBits(int bitsIn, boolean forceBits) {
        if (bitsIn != this.bits) {
            ...
            if (this.bits <= 4) {
                ...
                this.bits = MathHelper.log2DeBruijn((int)Block.BLOCK_STATE_IDS.size());
                if (forceBits) {
                    this.bits = bitsIn;
                }
            }
    ...
    public NibbleArray getDataForNBT(byte[] blockIds, NibbleArray data) {
        ...
        for (int i = 0; i < 4096; ++i) {
            int j = Block.BLOCK_STATE_IDS.get((Object)this.get(i));
            int k = i & 0xF;
            int l = i >> 8 & 0xF;
            int i1 = i >> 4 & 0xF;
            if ((j >> 12 & 0xF) != 0) {
    ...
    public void setDataFromNBT(byte[] blockIds, NibbleArray data, @Nullable NibbleArray blockIdExtension) {
        for (int i = 0; i < 4096; ++i) {
            ...
            int j1 = i1 << 12 | (blockIds[i] & 0xFF) << 4 | data.get(j, k, l);
            this.set(i, (IBlockState)Block.BLOCK_STATE_IDS.getByValue(j1));
        }
    }

    public int getSerializedSize() {
        return 1 + this.palette.getSerializedSize() + PacketBuffer.getVarIntSize((int)this.storage.size()) + this.storage.getBackingLongArray().length * 8;
    }
}
