...
public enum EnumFacing implements IStringSerializable
{
    ...
    public static EnumFacing byName(String name) {
        return name == null ? null : (EnumFacing)((Object)NAME_LOOKUP.get((Object)name.toLowerCase(Locale.ROOT)));
    }

    public static EnumFacing byIndex(int index) {
        return VALUES[MathHelper.abs((int)(index % VALUES.length))];
    }

    public static EnumFacing byHorizontalIndex(int horizontalIndexIn) {
        return HORIZONTALS[MathHelper.abs((int)(horizontalIndexIn % HORIZONTALS.length))];
    }

    public static EnumFacing fromAngle(double angle) {
        return EnumFacing.byHorizontalIndex(MathHelper.floor((double)(angle / 90.0 + 0.5)) & 3);
    }
    ...
    public static EnumFacing getDirectionFromEntityLiving(BlockPos pos, EntityLivingBase placer) {
        if (Math.abs((double)(placer.p - (double)((float)pos.p() + 0.5f))) < 2.0 && Math.abs((double)(placer.r - (double)((float)pos.r() + 0.5f))) < 2.0) {
            double d0 = placer.q + (double)placer.by();
            if (d0 - (double)pos.q() > 2.0) {
                return UP;
            }
            if ((double)pos.q() - d0 > 0.0) {
                return DOWN;
            }
        }
        return placer.bt().getOpposite();
    }
    ...
    static {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            NAME_LOOKUP.put((Object)enumfacing.getName2().toLowerCase(Locale.ROOT), (Object)enumfacing);
        }
    }
}
