...
public class PotionItems implements IFixableData {
   ...
   public NBTTagCompound fixTagCompound(NBTTagCompound var1) {
      if ("minecraft:potion".equals(☃.getString("id"))) {
         NBTTagCompound ☃ = ☃.getCompoundTag("tag");
         short ☃x = ☃.getShort("Damage");
         if (!☃.hasKey("Potion", 8)) {
            String ☃xx = POTION_IDS[☃x & 127];
            ☃.setString("Potion", ☃xx == null ? "minecraft:water" : ☃xx);
            ☃.setTag("tag", ☃);
            if ((☃x & 16384) == 16384) {
               ☃.setString("id", "minecraft:splash_potion");
            }
         }
