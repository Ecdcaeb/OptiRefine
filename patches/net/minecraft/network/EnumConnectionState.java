...
public enum EnumConnectionState {
   ...
   protected EnumConnectionState registerPacket(EnumPacketDirection var1, Class<? extends Packet<?>> var2) {
      BiMap<Integer, Class<? extends Packet<?>>> ☃ = this.directionMaps.get(☃);
      if (☃ == null) {
         ☃ = HashBiMap.create();
         this.directionMaps.put(☃, ☃);
      }

      if (☃.containsValue(☃)) {
         String ☃x = ☃ + " packet " + ☃ + " is already known to ID " + ☃.inverse().get(☃);
         LogManager.getLogger().fatal(☃x);
   ...
   public Integer getPacketId(EnumPacketDirection var1, Packet<?> var2) throws Exception {
      return (Integer)this.directionMaps.get(☃).inverse().get(☃.getClass());
   }

   @Nullable
   public Packet<?> getPacket(EnumPacketDirection var1, int var2) throws IllegalAccessException, InstantiationException {
      Class<? extends Packet<?>> ☃ = (Class<? extends Packet<?>>)this.directionMaps.get(☃).get(☃);
      return (Packet<?>)(☃ == null ? null : ☃.newInstance());
   }
   ...
   public static EnumConnectionState getById(int var0) {
      return ☃ >= -1 && ☃ <= 2 ? STATES_BY_ID[☃ - -1] : null;
   }

   public static EnumConnectionState getFromPacket(Packet<?> var0) {
      return STATES_BY_CLASS.get(☃.getClass());
   }
   ...
   static {
      for (EnumConnectionState ☃ : values()) {
         int ☃x = ☃.getId();
         if (☃x < -1 || ☃x > 2) {
            throw new Error("Invalid protocol ID " + Integer.toString(☃x));
         }

         STATES_BY_ID[☃x - -1] = ☃;

         for (EnumPacketDirection ☃xx : ☃.directionMaps.keySet()) {
            for (Class<? extends Packet<?>> ☃xxx : ☃.directionMaps.get(☃xx).values()) {
               if (STATES_BY_CLASS.containsKey(☃xxx) && STATES_BY_CLASS.get(☃xxx) != ☃) {
                  throw new Error("Packet " + ☃xxx + " is already assigned to protocol " + STATES_BY_CLASS.get(☃xxx) + " - can't reassign to " + ☃);
               }

               try {
                  ☃xxx.newInstance();
               } catch (Throwable var10) {
                  throw new Error("Packet " + ☃xxx + " fails instantiation checks! " + ☃xxx);
               }

               STATES_BY_CLASS.put(☃xxx, ☃);
            }
         }
      }
   }
